...
A $\kappa$-\emph{path} is an expression rooted at a variable in scope, that follows
\<final> field dereferences or list accesses, at most $\kappa$ times.
%
\begin{definition}[Paths]\label{def:path}
  ...
  \item if $\kappa\ge 1$ and $v\in\dom(\tau\at{p})$ then $v\in\mathbb{P}^\kappa\at{p}$, it has type $(\tau\at{p})(v)$
    and length $1$ (all variables in scope at $p$ are a $1$-path at $p$);
  \item if $\pi\in\mathbb{P}^\kappa\at{p}$ has type $C$ and length $l<\kappa$,
    and if $\mathtt{f}$ is a \<final> field of class $C$ of type $t$,
    already initialized at $p$,
    then $\pi.f\in\mathbb{P}^\kappa\at{p}$, it has type $t$ and length $l+1$
    (paths can be expanded by \<final> field dereference);
  \item if $\pi\in\mathbb{P}\at{p}$ has type \<List<$t$\texttt{>}> and length $l<\kappa$
    then $\pi\<[any]>\in\mathbb{P}^\kappa\at{p}$, it has type $t$ and length $l+1$
...
\texttt{this} is a $1$-path of type \texttt{Ponzi1},
\texttt{amount} is a $1$-path of type \texttt{int},
\texttt{this.investors} is a $2$-path of type \<List<Contract\texttt{>}> and
\texttt{this.investors[any]} is a $3$-path of type \<Contract>. Among them, only
\texttt{this.investors} is relevant.

In Def.~\ref{def:path}, the constraint on \<final> fields
simplifies the analysis, since it ensures that the value of a path
cannot be affected by the execution of code that modifies some fields.
The $\kappa$ limit, instead, is essential to define a finitely computable
...
\begin{definition}[Definite Aliasing Analysis]\label{def:definite_aliasing}
  ...
  $v\in\dom(\tau\at{p})$
  to a set of paths that are definitely alias of $v$ (\ie they hold the same
  ...
  %
  \begin{align*}
    \daa{c}(\sigma)&=\sigma[\res\mapsto\varnothing]\\
    \daa{v}(\sigma)&=\sigma[\res\mapsto\sigma(v)]\\
    \daa{\field{\exp}{f}}(\sigma)&=\sigma'[\res\mapsto\{\pi.f\mid\pi\in\sigma'(\res)\}]\quad\text{if $f$ is \<final>, where $\sigma'=\daa{\exp}(\sigma)$}
  \end{align*}
\end{definition}
