...
\begin{definition}[Paths]\label{def:path}
  ...
  \begin{itemize}
  \item if $\kappa\ge 1$ and $v\in\dom(\tau_{C.m})$ then $v\in\paths{C.m}$, it has type $(\tau_{C.m})(v)$
    and $\length(v)=1$ (all variables in scope in $C.m$ are a $1$-path in $C.m$);
...
\begin{definition}\label{def:survival}
  ...
  Given a command $\com$ occurring in $C.m$, then \emph{$\pi$ survives $\com$}, written $\pi\survives{\com}$, if
  the same condition as above holds and, moreover, $\com$ does not assign the starting
  variable of $\pi$, if any.
  This survival test is extended to a filter on sets $S$ of paths, as $S\survives X=\{\pi\in S\mid\pi\survives{X}\}$,
  and further extended pointwise to functions $\phi$ into sets of paths
...
\begin{definition}[Definite Aliasing Analysis]\label{def:definite_aliasing}
  Given a method $C.m$, the set of the \emph{abstract definite aliasing states in $C.m$} is
  \[
  \dastates{C.m}=\{\sigma\mid\sigma:\dom(\tau_{C.m})\mapsto\paths{C.m}\}
  \]
  The \emph{abstract definite aliasing denotation} of an expression
  $\exp$ occurring in $C.m$ is a function
  \[
  \da{C.m}{\exp}:\dastates{C.m}\mapsto\left(\dastates{C.m}\times\paths{C.m}\right)
  \]
  from the approximation of the pre-state to the approximation of the post-state and
  the approximation of the paths aliased to the expression. The annotation
  $C.m$ is dropped when clear from the context.
  The definition of $\da{}{\exp}$ is in Fig.~\ref{fig:daa_expressions}.
  The \emph{abstract definite aliasing denotation} of a command
  $\com$ occurring in $C.m$ is a function
  \[
  \da{C.m}{\com}:\dastates{C.m}\mapsto\dastates{C.m}
  \]
  from the approximation of the pre-state to the approximation of the post-state.
  The annotation $C.m$ is dropped when clear from the context.
  The definition of $\da{}{\com}$ is in Fig.~\ref{fig:daa_commands}.
\end{definition}
%
\begin{figure}[t]
  \begin{align*}
    ...
    \da{}{\readmap{\exp_1}{\exp_2}{\exp_3}}&=\pair{\sigma_3}{\varnothing}\\
    &\quad\text{where $\da{}{\exp_1}(\sigma)=\pair{\sigma_1}{\val_1}$,}\\
    &\quad\text{$\da{}{\exp_2}(\sigma_1)=\pair{\sigma_2}{\val_2}$}\\
    &\quad\text{and $\da{}{\exp_3}(\sigma_2)=\pair{\sigma_3}{\val_3}$}\\
    \da{}{\new{C'}{\exp_1,\ldots,\exp_n}}(\sigma)&=\pair{\sigma_n\survives{\new{C'}{\exp_1,\ldots,\exp_n}}\sigma'}{\varnothing}\\
    &\quad\text{where $\da{}{\exp_1}(\sigma)=\pair{\sigma_1}{\val_1}$,}\\
    &\quad\text{$\da{}{\exp_2}(\sigma_1)=\pair{\sigma_2}{\val_2}$}\\
    &\quad\vdots\\
    &\quad\text{and $\da{}{\exp_n}(\sigma_{n-1})=\pair{\sigma_n}{\val_n}$}\\
    \da{}{\size{\exp}}(\sigma)&=\pair{\sigma'}{\varnothing}\\
    &\quad\text{where $\da{}{\exp}(\sigma)=\pair{\sigma'}{\val}$}\\
    \da{}{\last{\exp}}(\sigma)&=\pair{\sigma'}{\{\pi\<[any]>\mid\pi\in\val\}\cap\paths{}}\\
    &\quad\text{where $\da{}{\exp}(\sigma)=\pair{\sigma'}{\val}$}\\
    \da{}{\random{\exp}}(\sigma)&=\pair{\sigma'}{\varnothing}\\
    &\quad\text{where $\da{}{\exp}(\sigma)=\pair{\sigma'}{\val}$}
  \end{align*}
...
\end{figure}
%
In Fig.~\ref{fig:daa}, the intersection with $\paths{}$ ensures that no illegal path
is generated (\ieinitial with non-\<final> fields, or longer than $\kappa$ or not
