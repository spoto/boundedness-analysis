...
\begin{definition}[Expressions and Commands]\label{def:exp_com}
  ...
  \begin{align*}
    \exp::=&\ \mathit{c}\mid\mathit{v}\mid\field{\exp}{f}\mid\op{\exp}{\exp}\mid\readmap{\exp}{\exp}{\exp}\\
    ...
    &\mid\ife{\exp}{\com}{\com}\mid\for{\mathit{v}}{\com}\mid\sequence{\com}{\com}
  \end{align*}
  ...
  line number and character position inside the line,
...
\begin{definition}[Side-Effects Analysis]\label{def:side_effect}
  ...
  \begin{align*}
    ...
    \sideeffect{\op{\exp_1}{\exp_2}}&=\sideeffect{\exp_1}\cup\sideeffect{\exp_2}\\
    &\quad\text{similarly, recursively, for the other expressions}
  \end{align*}
  \begin{align*}
    \sideeffect{\push{\exp_1\at{p}}{\exp_2}}&=\sideeffect{\exp_2}\cup\{t\}\quad\text{where $\tau\at{p}(\exp_1)=\<List<>t$\<\text{>}>}\\
    \sideeffect{\call{\exp_0\at{p}}{m}{\exp_1,\ldots,\exp_n}}&=\bigcup\limits_{j=1}^k\sideeffect{\com_j}\cup\bigcup\limits_{i=0}^{n}\sideeffect{\exp_i}\\
...
field dereferences or list accesses.
%
\begin{definition}[Paths]\label{def:path}
  The set $\mathbb{P}\at{p}$ of the \emph{paths at program point $p$} is the minimal set such that
  \begin{itemize}
  \item if $v\in\dom(\tau\at{p})$ then $v\in\mathbb{P}\at{p}$ and it has type $(\tau\at{p})(v)$
    (all variables in scope at $p$ are a path at $p$);
  \item if $\pi\in\mathbb{P}\at{p}$ has type $C$ and $\mathtt{f}$ is a field of class $C$ of type $t$,
    already initialized at $p$,
    then $\pi.f\in\mathbb{P}\at{p}$ and it has type $t$ (paths can be expanded by field dereference);
  \item if $\pi\in\mathbb{P}\at{p}$ has type \<List<$t$\texttt{>}>
    then $\pi\<[any]>\in\mathbb{P}\at{p}$ and it has type $t$ (paths can be expanded by list access).
  \end{itemize}
  A path is \emph{$\kappa$-relevant} if its type is \<List<$t$\texttt{>}>, it only dereferences
  \<final> fields and its depth
  (\ieinitial number of field dereferences and list accesses) is bounded by some constant $\kappa$.
  The (finite) set of all $\kappa$-relevant paths at $p$ is written as $\mathbb{RP}^\kappa\at{p}$. 
\end{definition}
%
For instance, at line~\ref{ponzi1:require} in Fig.~\ref{fig:ponzi1},
\texttt{this} is a path of type \texttt{Ponzi1},
\texttt{amount} is a path of type \texttt{int},
\texttt{this.investors} is a path of type \<List<Contract\texttt{>}> and
\texttt{this.investors[any]} is a path of type \<Contract>. Among them, only
the path \texttt{this.investors} is $\kappa$-relevant, for any $\kappa\ge 1$.
None of them is $0$-relevant.

In Def.~\ref{def:path}, the constraint on \<final> fields
simplifies the analysis, since the value of a relevant path
cannot be affected by the execution of code that modifies some fields.
The $\kappa$ limit, instead, will be essential to define a finitely computable
boundedness analysis, as shown later. Larger $\kappa$ lead to more precise analyses,
...
analysis. For simplicity, in the following examples,
we assume $\kappa=3$ and just say \emph{relevant}, meaning $3$-relevant.

A relevant path \emph{survives} the execution of some code\ie it keeps its value unchanged,
if it does not access any list that might be modified by that execution. Note how this
definition is simplified by the constraint that relevant paths only dereference
\<final> fields, which allows one to focus on side-effects on lists only.
%
\begin{definition}\label{def:survival}
  Given $\pi\in\mathbb{RP}^\kappa\at{p}$ and an expression $\exp\at{p}$, then
...
\end{definition}
%
For instance, the relevant path $\pi=\<this.investors>$ at
...
Instead, the relevant path $\pi=\<last.sales>$ at line~\ref{lottery2:push_round}
