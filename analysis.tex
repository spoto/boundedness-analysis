...
\begin{definition}\label{def:survival}
  Given $\pi\in\paths{C.m}$ and an expression $\exp$ occurring in $C.m$, then
  \emph{$\pi$ survives $\exp$} if, for all prefix $\pi'$ of $\pi$
  (\ieinitial  $\pi=\pi'\cdots$) of type \<List<$t'$\texttt{>}>
  and for all $t''\in\sideeffect{C.m}{\exp}$, the types
  $t'$ and $t''$ have no common subtype.
  A similar definition states when \emph{$\pi$ survives a command $\com$
  occurring in $C.m$}.
\end{definition}
%
...
\begin{definition}[Definite Aliasing Analysis]\label{def:definite_aliasing}
  ...
  It is defined as (we leave $C.m$ implicit to simplify the notation):
  %
  \begin{align*}
