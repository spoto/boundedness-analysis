...
\begin{example}\label{ex:side_effect}
  ...
  the execution of each of their constructors or methods.
  Consider Fig.~\ref{fig:side_effect_lottery1}, for instance.
  Its computation starts from $I_0$ such that $I_0(M)=\varnothing$ for each method $M$.
  Then we can compute
  $I_1(\<TicketBatchSale.TicketBatchSale>)=\sideeffect{\com}=\varnothing$,
  where $\com$ is the body of that constructor (Fig.~\ref{fig:lottery1}) and
  by the rules for $\assign{v}{\exp}$ and for $\sequence{\com}{\com}$ in
  Fig.~\ref{fig:side_effect}. Similarly,
  $I_1(\<Round.Round>)=\sideeffect{\com}=\varnothing$, where $\com$ is the body of
  that constructor, by the same rules as above
  and since $\sideeffect{\<sales=new List()>}=I_0(\<List.List>)=\varnothing$
  (we are abusing notation since Fig.~\ref{fig:lottery1} is not in the language
  in Sec.~\ref{sec:syntax}: the result actually holds for its transformation
  in that language).
  Moreover, $I_1(\<Lottery1.Lottery1>)=\{\<Round>\}$, by the same rules as above
  and since $\sideeffect{\<rounds.push(...)>}=\{\<Round>\}$ by the rule for \<push>
  in Fig.~\ref{fig:side_effect}. In the same way, it is
  $I_1(\<Lottery.buy>)=\{\<Round>,\<TicketBatchSale>\}$ and
  $I_1(\<Lottery.drawWinner>)=\varnothing$. The computation of $I_2$ leads to
  $I_2=I_1$, hence $I_1$ is the fixpoint, reported in Fig.~\ref{fig:side_effect_examples}.
  %
  \begin{figure}[t]
    \begin{center}
    ...
    \begin{subfigure}{\textwidth}
      ...
      \label{fig:side_effect_lottery1}
    \end{subfigure}
    \end{center}
...
\end{example}
%
The approximations in Fig.~\ref{fig:side_effect_examples} are correct
but sometimes imprecise. For instance, $\intsideeffect(\<Ponzi1.Ponzi1>)=\{\<Contract>\}$
is correct but could be improved
to a more precise $\intsideeffect(\<Ponzi1.Ponzi1>)=\varnothing$,
...
These imprecisions could be overcome with more complex analyses,
but the results in Fig.~\ref{fig:side_effect_examples} are good enough for our purposes.
In the following, we just assume that $\intsideeffect{}$ has been computed.
